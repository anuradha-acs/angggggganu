...
<?php
class Autonumber {
    protected static $tbl_name = "tblautonumber";
    ...
    function generate_autonumber($type="") {
        global $mydb;
        $mydb->setQuery("SELECT * FROM ".self::$tbl_name." WHERE CATEGORY = '{$type}' LIMIT 1");
        $cur = $mydb->loadSingleResult();
        
        if($cur) {
            $newNumber = $cur->PREFIX . str_pad($cur->NXTNUMBER, 4, '0', STR_PAD_LEFT);
            
            // Update next number
            $nextNum = $cur->NXTNUMBER + 1;
            $mydb->setQuery("UPDATE ".self::$tbl_name." SET NXTNUMBER = {$nextNum} WHERE ID = {$cur->ID}");
            $mydb->executeQuery();
